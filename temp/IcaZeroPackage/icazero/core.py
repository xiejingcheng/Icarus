...
class Variable:
    ...
    @property
    def T(self):
        return icazero.functions.transpose(self)
    ...
    def reshape(self, *shape):
        if len(shape) == 1 and isinstance(shape[0], (tuple, list)):
            shape = shape[0]
        return icazero.functions.reshape(self, shape)
    
    def sum(self, axis=None, keepdims = False):
        return icazero.functions.sum(self, axis, keepdims)
...
class Add(Function):
    ...
    def backward(self, gy):
        ...
        if self.x0_shape != self.x1_shape:
            gx0 = icazero.functions.sumTo(gx0, self.x0_shape)
            gx1 = icazero.functions.sumTo(gx1, self.x1_shape)
        return gx0, gx1
